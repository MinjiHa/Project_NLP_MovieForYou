...
5. 선택한 키워드와 관련있는 리뷰를 가진 영화 목록을 추천받습니다.<br>  내부적으로 Word2Vec을 사용하여 관련 키워드를 포함한 커스텀 키워드를 통해 코사인 유사도를 측정하였기 때문에 리뷰에 선택한 키워드가 꼭 포함되어 있지 않더라도 관련성이 높은 영화들이 추천됩니다.
